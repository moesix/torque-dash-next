...
/**
 * Resolves the vehicle fields that describe a session for the AI prompt.
 * The session's assigned Vehicle profile (plans/118) wins per-field; the
 * legacy Settings vehicle fields (set once in the old admin UI, never
 * per-car) are the fallback for any field the profile leaves unset. A
 * session with NO Vehicle resolves to the legacy fields verbatim, so prompt
 * output stays byte-identical to the pre-profile behavior.
 */
function resolveVehicleContext(session, settings) {
  const v = session?.Vehicle || null;
  return {
    year: v?.year ?? settings.vehicleYear,
    make: v?.make ?? settings.vehicleMake,
    model: v?.model ?? settings.vehicleModel,
    engineCc: v?.engineCc ?? settings.engineCc,
  };
}

function buildContext(session, settings, telemetrySample, pidKeys) {
  const lines = [];
  const vehicle = resolveVehicleContext(session, settings);
  ...
  if (vehicle.engineCc) lines.push(`Engine: ${vehicle.engineCc}cc`);
...
function buildAnalysisPrompt(session, settings, telemetrySample, pidKeys, dataQuality) {
  ...
  const vehicle = resolveVehicleContext(session, settings);
  const engineSizeStr = vehicle.engineCc
    ? `a ${vehicle.engineCc}cc engine`
    : 'this engine';
...
module.exports = { buildAnalysisPrompt, buildTelemetryCsv, buildContext, computeSummaryStats, formatSessionDuration, resampleTelemetry, detectBackfillGaps, buildDataQualityNote, resolveVehicleContext, GAP_ANALYSIS };
