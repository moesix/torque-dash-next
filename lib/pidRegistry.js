...
// Discovered key-sets are append-mostly per session; cache until ingest says
// otherwise. Long TTL is safe because flush() invalidates on every write.
const pidKeyCache = new UserCache({ ttl: 3600000, max: 5000 });
...
const PID_REGISTRY = {
  ...
  k6:       { fullName: 'Fuel Trim (Short Term Bank 1)',  shortName: 'STFT B1',   unit: '%' },
  k7:       { fullName: 'Fuel Trim (Long Term Bank 1)',   shortName: 'LTFT B1',   unit: '%' },
  kb:       { fullName: 'Intake Manifold Pressure',    shortName: 'MAP',       unit: 'kPa' },
  ...
  k14:      { fullName: 'O2 Sensor 1 Voltage (B1S1)',     shortName: 'O2S1',      unit: 'V' },
  kff1001:  { fullName: 'MAF-derived Speed Est',       shortName: 'MAF Speed', unit: 'km/h' },
  kff1005:  { fullName: 'GPS Longitude',              shortName: 'Lon',       unit: '°' },
  kff1006:  { fullName: 'GPS Latitude',               shortName: 'Lat',       unit: '°' },
...
module.exports = { PID_REGISTRY, discoverPidKeys, invalidatePidKeys };

/**
 * Discover all distinct k* PID keys present in a session's logs.
 * Returns an alphabetically sorted array of key strings (e.g. ['k5','kb','kc']).
 * Results are cached per session (pidKeyCache); ingestBuffer.flush() drops
 * entries via invalidatePidKeys whenever it successfully writes rows, so the
 * cache can only go stale if rows are modified outside this app.
 */
async function discoverPidKeys(sessionId, sequelize) {
    const cached = pidKeyCache.get(sessionId);
    ...
    pidKeyCache.set(sessionId, keys);
    return keys;
}

/**
 * Drop the cached PID key-set for one session. Called by ingestBuffer.flush()
 * after a successful write — any new row may have introduced a new key.
 */
function invalidatePidKeys(sessionId) {
    pidKeyCache.del(sessionId);
}
